...
class MeusChamadoList extends TPage
{
    private $form; // form
    ...
    public function __construct($param = null)
    {
        ...
        $dt_abertura_fim->setSize(150);
        $prioridade_id->setSize('100%');
        $dt_fechamento_final->setSize(150);
        ...
        $row2 = $this->form->addFields([new TLabel("Produto:", null, '14px', null, '100%'),$produto_id],[new TLabel("Categoria:", null, '14px', null, '100%'),$categoria_id]);
        $row2->layout = [' col-sm-6',' col-sm-6'];

        $row3 = $this->form->addFields([new TLabel("Situação:", null, '14px', null, '100%'),$status_id],[new TLabel("Prioridade:", null, '14px', null, '100%'),$prioridade_id]);
        $row3->layout = [' col-sm-6','col-sm-6'];

        $row4 = $this->form->addFields([new TLabel("Observação de abertura:", null, '14px', null),$observacao_finalizacao],[new TLabel("Observação fechamento:", null, '14px', null, '100%'),$observacao_abertura]);
        $row4->layout = [' col-sm-6','col-sm-6'];

        $row5 = $this->form->addFields([new TLabel("Abertura:", null, '14px', null, '100%'),$dt_abertura,new TLabel("até", null, '14px', null),$dt_abertura_fim],[new TLabel("Fechamento:", null, '14px', null, '100%'),$dt_fechamento,new TLabel("até", null, '14px', null),$dt_fechamento_final]);
        ...
        $this->filter_criteria->add(new TFilter('solicitante_id', 'in', "(SELECT id FROM cliente WHERE system_user_id = '{$filterVar}')"));
        ...
        $action_onShow->setImage('fas:search #478fca');
        ...
        $action_NotaForm_onShow->setImage('far:comment-dots #FF5722');
        $action_NotaForm_onShow->setField(self::$primaryKey);
        $action_NotaForm_onShow->setDisplayCondition('MeusChamadoList::canNota');
        ...
        $button_cadastrar->setAction(new TAction(['NovoChamadoForm', 'onShow']), "Cadastrar");
        ...
        $btnShowCurtainFilters->setAction(new TAction(['MeusChamadoList', 'onShowCurtainFilters']), "Filtros");
        ...
        $button_atualizar->setAction(new TAction(['MeusChamadoList', 'onRefresh']), "Atualizar");
        ...
        $button_limpar_filtros->setAction(new TAction(['MeusChamadoList', 'onClearFilters']), "Limpar filtros");
        ...
        $dropdown_button_exportar->addPostAction( "CSV", new TAction(['MeusChamadoList', 'onExportCsv'],['static' => 1]), 'datagrid_'.self::$formName, 'fas:file-csv #00b894' );
        $dropdown_button_exportar->addPostAction( "XLS", new TAction(['MeusChamadoList', 'onExportXls'],['static' => 1]), 'datagrid_'.self::$formName, 'fas:file-excel #4CAF50' );
        $dropdown_button_exportar->addPostAction( "PDF", new TAction(['MeusChamadoList', 'onExportPdf'],['static' => 1]), 'datagrid_'.self::$formName, 'far:file-pdf #e74c3c' );
        $dropdown_button_exportar->addPostAction( "XML", new TAction(['MeusChamadoList', 'onExportXml'],['static' => 1]), 'datagrid_'.self::$formName, 'far:file-code #95a5a6' );
        ...
        if(empty($param['target_container']))
        {
            $container->add(TBreadCrumb::create(["Cliente","Meus Chamados"]));
        }
    ...
    public static function canNota($object)
    {
        try 
        {
            if($object->status->estado_final == 'F')
            {
                return true;
            }
    ...
    public static function onShowCurtainFilters($param = null) 
    {
        try 
        {
            ...
            $page->setProperty('page-name', 'MeusChamadoListSearch');
            $page->setProperty('page_name', 'MeusChamadoListSearch');
