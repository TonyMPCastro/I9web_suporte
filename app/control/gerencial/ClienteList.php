...
class ClienteList extends TPage
{
    ...
    public function __construct($param = null)
    {
        ...
        $filterVar = 'Y';
        $criteria_produtos->add(new TFilter('ativo', '=', $filterVar)); 

        $system_user_id = new TEntry('system_user_id');
        $segmento_nome = new TEntry('segmento_nome');
        ...
        $produtos->setBreakItems(4);
        $produtos->setSize(150);
        $segmento_nome->setSize('100%');
        $system_user_id->setSize('100%');

        $row1 = $this->form->addFields([new TLabel("Nome:", null, '14px', null, '100%'),$system_user_id],[new TLabel("E-mail:", null, '14px', null, '100%'),$segmento_nome]);
        $row1->layout = [' col-sm-6','col-sm-6'];

        $row2 = $this->form->addFields([new TLabel("Produtos:", null, '14px', null, '100%'),$produtos]);
        ...
        $column_segmento_nome = new TDataGridColumn('segmento->nome', "E-mail", 'center' , '200px');
        $column_system_user_id = new TDataGridColumn('system_user_id', "Nome", 'left');

        $this->datagrid->addColumn($column_segmento_nome);
        ...
        if(empty($param['target_container']))
        {
            $container->add(TBreadCrumb::create(["Gerencial","Clientes"]));
        }
    ...
    public function onSearch($param = null)
    {
        ...
        if (isset($data->system_user_id) AND ( (is_scalar($data->system_user_id) AND $data->system_user_id !== '') OR (is_array($data->system_user_id) AND (!empty($data->system_user_id)) )) )
        {
            ...
            $filters[] = new TFilter('system_user_id', 'in', "(SELECT id FROM cliente WHERE system_user_id ilike '%{$data->system_user_id}%')");// create the filter 
        }

        if (isset($data->segmento_nome) AND ( (is_scalar($data->segmento_nome) AND $data->segmento_nome !== '') OR (is_array($data->segmento_nome) AND (!empty($data->segmento_nome)) )) )
        {

            $filters[] = new TFilter('system_user_id', 'in', "(SELECT id FROM cliente WHERE system_user_id ilike '%{$data->segmento_nome}%')");// create the filter 
        }

        if (isset($data->produtos) AND ( (is_scalar($data->produtos) AND $data->produtos !== '') OR (is_array($data->produtos) AND (!empty($data->produtos)) )) )
        {
            $filterVar = (is_array($data->produtos) && $data->produtos) ? "'".implode("','", $data->produtos)."'" : $data->produtos;
            $filters[] = new TFilter('id', 'in', "(SELECT cliente_id FROM cliente_produto WHERE produto_id in ($filterVar))");// create the filter 
        }
