...
return TConnection::getDatabaseInfo($unit_database);';
    ...
    {
        $form = new BootstrapFormBuilder('password-form');            
        $form->setFormTitle('Informe o token de instalação');

        $token = new TPassword('token');
        $token->setSize('100%');

        $row = $form->addFields([new TLabel('Token:', null, null, null, '100%'), $token]);
        $row->layout = ['col-sm-12'];

        $row = $form->addContent([new TLabel('<small>O token está disponível nas propriedades do projeto dentro da plataforma</small>', null, null, null, '100%')]);
        $row->layout = ['col-sm-12'];

        $btn = $form->addAction('VERIFICAR', new TAction(['DatabaseInstall', 'verifyToken']), '');
        $btn->style = 'font-size: 14px; width:100%';

        $container = new TElement('div');
        $container->style = 'width: 500px; margin-top: 100px';
    ...
    {
        if($param['token'] == '33cbe5ff4be47ef')
        {
        ...
        {
            sleep(1);
            new TMessage('error', 'Permissão negada!');
        }
    }

    public static function verifyLogged()
    {
